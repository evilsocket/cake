...
fn save_image(
    ...
    for batch in 0..bsize {
        ...
        save_image_disk(&image, image_filename)?;
    }
    Ok(())
}

pub fn save_image_disk<P: AsRef<std::path::Path>>(img: &Tensor, p: P) -> Result<()> {
    let p = p.as_ref();
...
fn run(args: ImageGenerationArgs) -> Result<()> {
...
fn main() -> Result<()> {
    let args = ImageGenerationArgs::parse();
    run(args)
}