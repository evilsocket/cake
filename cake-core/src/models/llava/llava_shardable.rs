...
/// Dispatches layer names to the appropriate LLaVA component:
/// - `"llava-vision"` → LlavaVision (CLIP + MM projector)
/// - `"model.layers.N"` or `"language_model.model.layers.N"` → Transformer block
#[derive(Debug)]
...
impl Forwarder for LlavaShardable {
        ...
        Self: Sized,
    {
        let model: Box<dyn Forwarder> = match name.as_str() {
            ...
            _ => {
                // Assume it's a transformer layer name
