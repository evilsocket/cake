...
impl Model {
    ...
    pub async fn next_token(&mut self, index: usize) -> Result<Token> {
        ...
        let is_eos: bool = Some(next_token) == self.eos_token_id;
        let text = self.tokenizer.next_token(next_token)?;
        let id = next_token;
        Ok(Token { id, text, is_eos })
    }

    pub fn final_token(&self) -> Result<Option<String>> {
