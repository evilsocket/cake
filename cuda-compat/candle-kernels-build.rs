...
fn main() {
    ...
    // Only include WMMA kernels if the .cu files exist (removed on pre-Ampere GPUs)
    let mut moe_paths = vec!["src/moe/moe_gguf.cu"];
    if std::path::Path::new("src/moe/moe_wmma.cu").exists() {
        moe_paths.push("src/moe/moe_wmma.cu");
    }
    if std::path::Path::new("src/moe/moe_wmma_gguf.cu").exists() {
        moe_paths.push("src/moe/moe_wmma_gguf.cu");
    }
    let moe_builder = moe_builder.kernel_paths(moe_paths);
