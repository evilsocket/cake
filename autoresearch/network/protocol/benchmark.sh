#!/bin/bash
# benchmark.sh — Fixed harness for network protocol autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: message serialization, tensor wire format, pipe roundtrips,
#           zstd compression, CRC32 checksums, authentication handshake,
#           discovery packets, topology management.
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/network/protocol/benchmark.sh
...
# Protocol + serialization + discovery + topology + auth benchmarks
BENCH_FILTER="single_op|batch_|hello_|worker_info|layer_assignment|pipe_roundtrip|large_tensor|buf_reuse|zstd_|crc32_|model_data|raw_tensor|cluster_hash|max_layers|encode_decode_packet|total_tflops|get_node|all_worker|is_text_model|auto_assign|compute_assignments|estimate_tflops|max_layers_for_gpus|compute_hmac|constant_time|auth_handshake"
