#!/bin/bash
# benchmark.sh — Fixed harness for TTS models autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: VibeVoice (prediction head, DDPM, connectors, streaming cache),
#           LuxTTS (activations, BiasNorm, mel spectrogram, Euler solver),
#           WAV I/O (encode, decode, save, load).
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/models/tts/benchmark.sh
...
# Run TTS-specific benchmarks: VibeVoice + LuxTTS + WAV
BENCH_FILTER="prediction_head|ddpm|acoustic_connector|semantic_connector|streaming_cache|swoosh|bias_norm|mel_spectrogram|euler_step|encode_wav|decode_wav|save_wav|load_wav"
