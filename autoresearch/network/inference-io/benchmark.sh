#!/bin/bash
# benchmark.sh — Fixed harness for inference I/O autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: tensor encode/decode, pipe roundtrips with buffer reuse,
#           batch encoding, RawTensor conversion — all on the per-token hot path.
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/network/inference-io/benchmark.sh
...
# Per-token hot path benchmarks: message encode/decode, pipe roundtrips, tensor conversion
BENCH_FILTER="single_op|batch_encode|batch_decode|batch_layers|hello_roundtrip|worker_info|pipe_roundtrip|buf_reuse|large_tensor|raw_tensor"
START_NS=$(date +%s%N 2>/dev/null || python3 -c "import time; print(int(time.time()*1e9))")

if ! DIVAN_SAMPLE_COUNT=5 cargo bench -p cake-core -- "$BENCH_FILTER" > "$BENCH_OUT" 2>&1; then
