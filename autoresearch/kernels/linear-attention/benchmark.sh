#!/bin/bash
# benchmark.sh — Fixed harness for linear attention autoresearch.
...
echo "=== CLIPPY ===" >&2
CLIPPY_PASS="FAIL"
if cargo clippy -p cake-core --lib --tests -- -D warnings 2>&1 | tail -5 >&2; then CLIPPY_PASS="PASS"; fi
echo "Clippy: $CLIPPY_PASS" >&2
echo "=== TESTS ===" >&2
TEST_PASS="FAIL"
if cargo test -p cake-core --lib 2>&1 | tail -10 >&2 \
   && cargo test -p cake-core --test unit 2>&1 | tail -10 >&2 \
   && cargo test -p cake-core --test protocol 2>&1 | tail -10 >&2; then TEST_PASS="PASS"; fi
echo "Tests: $TEST_PASS" >&2
echo "=== BENCHMARK ===" >&2
BENCH_OUT=$(mktemp)
trap "rm -f $BENCH_OUT 2>/dev/null" EXIT
BENCH_FILTER="linear_attn|qwen3_5.*block"
...
echo "Benchmark output:" >&2; cat "$BENCH_OUT" >&2
echo ""; echo "Score: ${SCORE} ns (wall-clock)" >&2
