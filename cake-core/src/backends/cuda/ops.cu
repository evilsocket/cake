...
extern "C" __global__ void silu_mul_f32(
    ...
    const unsigned int vec_n = numel >> 2;
    const float4 *x4 = reinterpret_cast<const float4*>(x);
    const float4 *y4 = reinterpret_cast<const float4*>(y);
    float4 *o4 = reinterpret_cast<float4*>(out);
    for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
         i < vec_n; i += blockDim.x * gridDim.x) {
        float4 xv = x4[i], yv = y4[i], r;
    ...
    for (unsigned int i = (vec_n << 2) + blockIdx.x * blockDim.x + threadIdx.x;
         i < numel; i += blockDim.x * gridDim.x) {
        out[i] = silu_mul_fwd(x[i], y[i]);
    }
}
...
// Vectorized f32 stable_softplus
...
extern "C" __global__ void add3_f32(
    ...
    for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
         i < vec_n; i += blockDim.x * gridDim.x) {
        float4 av = a4[i], bv = b4[i], cv = c4[i], r;
        r.x = av.x + bv.x + cv.x;
        r.y = av.y + bv.y + cv.y;
        r.z = av.z + bv.z + cv.z;
        r.w = av.w + bv.w + cv.w;
