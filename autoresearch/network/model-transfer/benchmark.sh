#!/bin/bash
# benchmark.sh — Fixed harness for model transfer autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: zstd compression/decompression, CRC32 checksums, model data chunk
#           serialization roundtrips (compressed and uncompressed).
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/network/model-transfer/benchmark.sh
...
# Model transfer benchmarks: compression, checksums, chunk roundtrips
BENCH_FILTER="zstd_compress|zstd_decompress|crc32_checksum|model_data_chunk"
