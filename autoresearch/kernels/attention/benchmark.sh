#!/bin/bash
# benchmark.sh — Fixed harness for attention kernel autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: attention forward, generation step, QK norm, sliding window,
#           GatedDeltaNet linear attention, cache operations.
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/kernels/attention/benchmark.sh
...
echo "=== CLIPPY ===" >&2
CLIPPY_PASS="FAIL"
if cargo clippy -p cake-core --lib -- -D warnings 2>&1 | tail -5 >&2; then
    CLIPPY_PASS="PASS"
fi
...
BENCH_FILTER="attention|linear_attn|cache"
