#!/bin/bash
# benchmark.sh — Fixed harness for CPU backend autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: CPU-path fused operations + inference primitives.
# Requires: No special hardware — CPU backend is always available.
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/backends/cpu/benchmark.sh
...
# Run CPU-path benchmarks: fused ops (CPU) + backend ops + attention
BENCH_FILTER="fused_.*_cpu|backend_ops|attention"
