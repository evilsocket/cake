// MSL CustomOp structs and helpers are compiled but temporarily unused while
// ops are reverted to candle tensor ops for correctness validation.
#![allow(dead_code)]
...
impl candle_core::CustomOp3 for MetalAddRmsNorm {
    ...
    fn metal_fwd(
        &self,
        s_a: &candle_core::MetalStorage, l_a: &Layout,
        s_b: &candle_core::MetalStorage, l_b: &Layout,
        s_w: &candle_core::MetalStorage, l_w: &Layout,
    ) -> Result<(candle_core::MetalStorage, Shape)> {
        ...
        let pipeline = PIPELINE_CACHE.get_or_create(device, kernel_name)?;
        // Packed output: [residual | normed] = 2 * el elements
        ...
        let group = objc2_metal::MTLSize { width: tg_width, height: 1, depth: 1 };
        encoder.dispatch_threads(grid, group);
        // Return packed buffer as flat shape [2 * el]
