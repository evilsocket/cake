...
pub struct CausalSelfAttention {
    ...
    size_q: usize,
    size_kv: usize,
    /// Optional RmsNorm weight applied to Q (after reshape unless pre_reshape_qk_norm).
    q_norm_weight: Option<Tensor>,
    /// Optional RmsNorm weight applied to K (after reshape unless pre_reshape_qk_norm).
    k_norm_weight: Option<Tensor>,
    /// Epsilon for QK-norm (only meaningful when q_norm_weight/k_norm_weight are Some).
    qk_norm_eps: f32,
...
impl CausalSelfAttention {
    ...
    pub fn load_custom(
        ...
        Ok(Self {
            ...
            q_norm_weight,
            k_norm_weight,
            qk_norm_eps: cfg.rms_norm_eps as f32,
