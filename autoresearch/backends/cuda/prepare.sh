#!/bin/bash
# prepare.sh — One-time setup for CUDA backend autoresearch.
# Requires: NVIDIA GPU with CUDA support.
#
# Usage: bash autoresearch/backends/cuda/prepare.sh
...
echo "========================================"
echo "  Cake Autoresearch — CUDA Backend"
echo "========================================"
echo ""
...
if ! command -v nvcc &>/dev/null && [ -z "${CUDA_HOME:-}" ]; then
    echo "WARNING: nvcc not found and CUDA_HOME not set."
    echo "Set CUDA_HOME to your CUDA installation (e.g. /usr/local/cuda-12.4)"
fi

echo "Building cake-core with CUDA..."
if ! cargo build -p cake-core --features "$FEATURES" 2>&1 | tail -5; then
    echo "ERROR: Build failed. Ensure CUDA toolkit is installed."
    exit 1
fi
echo "Build: OK"
echo ""
...
echo "  Saved to: autoresearch/backends/cuda/baseline.txt"
echo "  TSV: autoresearch/backends/cuda/experiments.tsv"
echo "========================================"
echo ""
