#!/bin/bash
# benchmark.sh — Fixed harness for CUDA backend autoresearch.
# DO NOT MODIFY. This is the ground truth measurement.
#
# Measures: GPU-path fused operations via CUDA backend.
# Requires: NVIDIA GPU with CUDA support.
# Metric: total benchmark wall-clock time in nanoseconds (lower is better).
#
# Usage: bash autoresearch/backends/cuda/benchmark.sh
...
# Run GPU-path benchmarks: fused ops + attention + blocks
BENCH_FILTER="fused_.*_gpu|attention|transformer_block"
