...
echo "========================================"
echo "  Cake Autoresearch — Linear Attention (GDN)"
echo "========================================"
echo ""
cd "$PROJECT_DIR"
echo "Building cake-core..."
if ! cargo build -p cake-core 2>&1 | tail -5; then echo "ERROR: Build failed"; exit 1; fi
echo "Build: OK"; echo ""
echo "Running tests..."
if ! cargo test -p cake-core --lib 2>&1 | tail -10 \
   || ! cargo test -p cake-core --test unit 2>&1 | tail -10 \
   || ! cargo test -p cake-core --test protocol 2>&1 | tail -10; then echo "ERROR: Tests failed"; exit 1; fi
echo "Tests: OK"; echo ""
