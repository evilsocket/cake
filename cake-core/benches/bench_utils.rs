use cake_core::backends::ComputeBackend;
...
fn fused_silu_mul_cpu(bencher: divan::Bencher, size: usize) {
    ...
    let backend = cake_core::backends::CpuBackend::new();
...
fn fused_depthwise_conv1d_silu_cpu(bencher: divan::Bencher, channels: usize) {
    ...
    let backend = cake_core::backends::CpuBackend::new();
    bencher.bench_local(|| {
        backend.depthwise_conv1d_silu(&window, &weight, kernel_size, channels)
            .unwrap()
    });
}
...
fn fused_depthwise_conv1d_bias_cpu(bencher: divan::Bencher, channels: usize) {
    ...
    let backend = cake_core::backends::CpuBackend::new();
    bencher.bench_local(|| {
        backend.depthwise_conv1d_bias(&padded, &weight, &bias, kernel_size, channels)
            .unwrap()
    });
}
